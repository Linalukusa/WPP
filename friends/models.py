...
class FriendRequest(models.Model):
    """ This class represents a friend request. When a user send a friend request, an
        ...
        Args:
            to_user (Profile): the user that is the target of the request
            from_user (Profile): the user that sent the request
            created: the time that the request is created
        """
    to_user = models.ForeignKey(Profile, related_name='to_user', on_delete=models.PROTECT)
    from_user = models.ForeignKey(Profile, related_name='from_user', on_delete=models.PROTECT)
